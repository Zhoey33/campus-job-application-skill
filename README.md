...
招聘表单提供内推或员工推荐选项时，Skill 默认选择内推；需要代码时，会主动从小红书和其他公开来源查找并核验适用于当前公司、届次和岗位的内推码，填写后确认网站接受。用户不需要为是否使用公开内推码增加一次审核。

Skill 只采用公开、真实且范围匹配的代码，不会私信、关注、加群、付费或提交个人信息换码，也不会编造推荐人或推荐关系。检查小红书和其他合理公开来源后仍找不到有效代码时，Skill 会保留未提交状态并说明准确缺口，不会悄悄改用普通网申。

## 多志愿处理

同一招聘系统通常让多个志愿共享候选人档案。Skill 会在第一志愿完成个人信息、教育经历、简历和通用附件的完整填写与复核；第二及后续志愿优先复用这些信息，只处理岗位专属问题、志愿顺序、差异附件和内推信息，不重复运行整页填表。
