# campus-job-application-workflow

一个面向 Codex Desktop 的端到端校招投递 Skill。它从环境初始化和招聘官网登录开始，自动完成官方岗位检索、投递数量核验、来个简历插件填表、受限字段文本优化、附件检查、表单复核、提交结果回读和飞书个人投递台账维护。用户不需要逐步指挥浏览器操作，只需处理少数必要的人工检查点。

## 端到端流程与人工检查点

在一次正常投递中，Skill 会持续推进各阶段，并尽量把人工介入收敛到以下三个检查点：

1. **登录检查点**：用户完成扫码、验证码、账号密码输入或浏览器权限确认；Skill 随后自行确认登录状态，并读取最大投递数、已投数量和剩余额度。
2. **岗位检查点**：Skill 完成官网岗位全量核验后，用户确认要投递的岗位及准确志愿顺序。确认后即视为允许上传本地资料文件夹中的简历，并使用“来个简历”完成对应表单，不再重复询问上传和填写授权。
3. **提交检查点**：Skill 填写并复核表单、处理附件后停在最终提交前，由用户一次审核全部已确认志愿并明确是否整组提交。用户可以在同一次回复中提出修改，或授权按志愿顺序连续提交。

除以上检查点以及无法从现有资料确定的个人事实外，打开官网、导航页面、筛选岗位、整理标签、上传附件、调用插件、检查必填项、读取提交结果和维护台账均由 Skill 自动完成。首次安装依赖和设置插件只在初始化时处理，后续运行会复用已保存的本地配置与登录状态。

## 默认内推策略

招聘表单提供内推或员工推荐选项时，Skill 默认选择内推；需要代码时，会为第一志愿主动从小红书和其他公开来源查找并核验适用于当前公司、届次和岗位的内推码，填写后确认网站接受。第二及后续志愿直接复用这个已验证代码，不再重复搜索；只有代码被官网拒绝或原始来源明确不适用于后续岗位时才重新核验。用户不需要为是否使用公开内推码增加一次审核。

Skill 只采用公开、真实且范围匹配的代码，不会私信、关注、加群、付费或提交个人信息换码，也不会编造推荐人或推荐关系。检查小红书和其他合理公开来源后仍找不到有效代码时，Skill 会保留未提交状态并说明准确缺口，不会悄悄改用普通网申。

## 多志愿处理

同一招聘系统通常让多个志愿共享候选人档案。Skill 会在第一志愿完成个人信息、教育经历、简历、通用附件和内推码的完整填写与复核；第二及后续志愿优先复用这些信息，只处理岗位专属问题、志愿顺序和差异附件，不重复运行整页填表或搜索内推信息。

用户一次授权整组志愿提交后，Skill 会按确认顺序逐个提交并分别回读结果，每成功一个志愿就重新检查剩余额度，再自动继续下一个。只有出现新增必填事实、验证码、额度或顺序变化、岗位失效或提交异常时才暂停。

## 首次使用

Skill 会在第一次运行时：

1. 检查并在需要时安装官方`lark-cli`及其配套 Skills；
2. 检查 Chrome 是否安装“来个简历”，缺失时从[来个简历官网](https://cv.playoffer.cn/)进入官方安装流程；
3. 让用户完成插件登录、同步和简历档案准备；
4. 一次性取得目标届次、岗位方向、飞书个人台账和本地资料文件夹绝对路径；
5. 创建本地`context.md`并生成`references/current-profile-field-map.md`。

后续运行会复用这些信息，只在用户更改、路径失效或插件档案切换时更新。公司始终从当前任务消息中取得，不会持久化为默认公司。

## 准备本地资料文件夹

首次使用前，请把网申可能用到的资料集中放进同一个本地文件夹，例如：

- 当前使用的 PDF 简历；
- 证件照；
- 成绩单；
- 英语、计算机或职业资格证书；
- 获奖证明及招聘网站可能要求的其他附件。

然后只需要把这个文件夹的绝对路径告诉 Codex，不必逐个提供文件路径。Skill 会根据文件名和格式建立附件清单；如果同类文件有多个且无法确定使用哪一个，Codex 才会列出候选文件名请你选择。

## 隐私

以下运行时文件已被`.gitignore`排除，不应提交到 GitHub：

- `context.md`
- `references/current-profile-field-map.md`
- `references/supplemental-information.md`

公开仓库只包含空白模板。不要在 Issue、提交信息或示例中粘贴个人简历、联系方式、飞书私有链接或本地路径。

## 依赖

- Codex Desktop 与 Computer Use；
- Chrome；
- 官方[`lark-cli`](https://github.com/larksuite/cli)；
- Chrome 扩展“来个简历”；
- 配套 Skill：`laige-resume-autofill`。

## 安装

将本目录放入 Codex Skills 目录，并保持目录名为`campus-job-application-workflow`。同时安装`laige-resume-autofill`。重新启动 Codex 或刷新 Skills 后即可使用。

## 在 Codex 中使用

安装完成后，不需要记住 Skill 的英文名称或输入命令。直接在 Codex 中说：

```text
按照校招投递 Skill 的全流程，投一下 XX 公司的校招。
```

Codex 会自动调用本 Skill。第一次使用时会先完成环境检查、插件设置和默认信息初始化；后续使用会复用本地上下文，并从当前消息中的`XX 公司`确定本次任务范围。

也可以在同一句话中覆盖默认届次或岗位方向，例如：

```text
按照校招投递 Skill 的全流程，投一下 XX 公司的 2027 届校招，找财务和审计方向的岗位。
```

首次初始化中的依赖安装、浏览器扩展权限确认、扫码登录和飞书授权仍受本机与宿主的权限规则约束。最终投递必须由用户对当前公司和岗位明确授权。
