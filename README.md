...
| 提交确认 | 审核已经准备好的志愿并授权提交 | 按顺序提交、逐项回读并更新台账 |
...
| 多志愿 | 第一志愿完整填写；后续志愿复用共享档案。若共享要在首志愿提交后才生效，则先交审并提交首志愿，再准备剩余志愿 |
...
通常全部志愿可以在一次提交检查点统一审核；若招聘系统要在第一志愿提交后才开放共享候选人档案，提交检查点会分阶段进行。后续阶段只审核岗位和附件差异，不重复审核已经确认的个人信息。
